...
$token = $_GET['token'];    // ? this returns everything before the _ character which is the original token
$original_token = strstr($token, "_", true);

$users_email = hex2bin(str_replace("_", "", strstr($token, "_")));

// ? echo "Original token = $original_token <br> User email = $users_email";

// ? update the user 
$query = $pdo->exec("UPDATE `users` SET `email_verify_status` = 1 WHERE `email` = '$users_email' AND `email_verification_token` = '$original_token'");
?>
							...
							<div class="row g-0">
								...
								<div class="col-lg-6 order-1">
									<div class="p-4 p-sm-7">
										...
										<!-- Title -->
										<h1 class="mb-2 fs-2">Email verified</h1>
										<p class="mb-0">Your email adress has been successful verified</p>
